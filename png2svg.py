...
def conversion_worker(png_file_path):
    log.info(f"{datetime.datetime.now()} Converting {png_file_path}")
    image = Image.open(png_file_path).convert("RGBA")
    # png to svg calculation
    svg_image = rgba_image_to_svg_contiguous(image)
    svg_file_path = png_file_path.replace(".png", ".svg")
    # remove existing svg file to avoid name conflict
    if os.path.exists(svg_file_path):
        os.remove(svg_file_path)
    # write new svg data to file
    with open(svg_file_path, "w") as text_file:
        text_file.write(svg_image)
    # remove png file
    os.remove(png_file_path)


def multiprocess_conversion(target_dir: pathlib.Path):
    if target_dir.is_dir():
        pool = multiprocessing.Pool(multiprocessing.cpu_count())
        files = os.listdir(target_dir)
        for file in files:
            if file.endswith(".png"):
                pool.apply_async(conversion_worker, (os.path.join(target_dir, file),))
        pool.close()
        pool.join()
